...
describe BinaryGame do
  ...
  describe '#display_turn_order' do
    ...
    before do
      ...
      allow(game_turn_order).to receive(:display_guess).with(turn_display)
    ...
    it 'increases guess_count by one' do
      guess_count_before = game_turn_order.instance_variable_get(:@guess_count)
      game_turn_order.display_turn_order(turn_display)
      guess_count_after = game_turn_order.instance_variable_get(:@guess_count)
      expect(guess_count_after).to eq(guess_count_before + 1)
    end

    # Nicer syntax for the test from the solutions
    # However, it's tough to get the spacing right
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(turn_display) }.to change { game_turn_order.instance_variable_get(:@guess_count) }.by(1)
    end
