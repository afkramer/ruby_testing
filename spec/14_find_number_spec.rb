...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_avg7) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        guess = game_avg7.make_guess
        expect(guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_avg8) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        guess = game_avg8.make_guess
        expect(guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_avg1) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        allow(number_guessing).to receive(:value).and_return(2)
        guess = game_avg1.make_guess
        expect(guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_avg3) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        allow(number_guessing).to receive(:value).and_return(3)
        guess = game_avg3.make_guess
  ...
  describe '#game_over?' do
    let(:random_number) { double('random_number') }
    subject(:game_status) { described_class.new(0, 15, random_number, 7) }
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        ...
        expect(game_status).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      it 'is not game over' do
        ...
        expect(game_status).to_not be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      # execute the method here then check if min and max were updated?
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:edge_case_min) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        edge_case_min.update_range
        expect(edge_case_min.min).to eq(edge_case_min.max)
        expect(edge_case_min.min).to eq(8)
      end

      it 'does not update max' do
        max_before = edge_case_min.max
        edge_case_min.update_range
        expect(edge_case_min.max).to eq(max_before)
      end
    end

    let(:low_random_number) { double('low_random_number', value: 2) }
    context 'when the guess is 3, min = 2, and max = 6' do
      subject(:edge_case_max) { described_class.new(2, 6, low_random_number, 3) }
      it 'does not update min' do
        min_before = edge_case_max.min
        edge_case_max.update_range
        expect(edge_case_max.min).to eq(min_before)
      end

      it 'updates max to same as min' do
        edge_case_max.update_range
        expect(edge_case_max.min).to eq(edge_case_max.max)
      end
    end
  end
end
