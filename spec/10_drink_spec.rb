...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:favorite_drink) { described_class.new('Coolatta') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(favorite_drink.type).to eq('Coolatta')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(favorite_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:favorite_drink) { described_class.new('frappe')}

      # remove the 'x' before running this test
      it 'is full' do
        expect(favorite_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:small_drink) { described_class.new('orange juice', 4) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(small_drink).to_not be_full
      end
    end
  end
end
